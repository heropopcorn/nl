...
| Input | Action |
| --- | --- |
| WASD / arrow keys | Walk (**default** play mode) |
| Shift | Sprint |
| `1` / `2` / `3` | Play / Water / Path editor modes |
| `B` | Load a custom ground PNG (`user://custom_ground.png`) |
| Mouse wheel or `+` / `-` | Zoom |
| Right-drag or middle-drag | Pan camera (works in every mode) |
| C or Home | Recenter camera on the player |
| `P` | Play or stop the walk polyline |
| F1 | Toggle the on-screen hint |
| F3 | Toggle collision debug |

The right-hand **Runtime editor** panel has Load PNG, water brush/polygon, and path play. Full table: [`docs/custom-editor-mvp.md`](docs/custom-editor-mvp.md).

The stream uses a canvas shader on a water mask (generated by default, paintable at runtime). Wooden plank bridges on the map are gaps in that mask so you can cross.
...
├── World (Node2D, y-sort)
│   ├── Terrain                   approved or user:// ground
...
│   └── MapBounds
├── PathPreview / water gizmo    runtime editor overlays
└── HUD
    ├── ControlsHint
    ├── EditorPanel              scripts/runtime_editor.gd
    └── PngFilePicker
```
...
## Custom ground / water / path (MVP)

Runtime editor (not a separate web app): swap the terrain PNG, paint the water mask, edit a walk polyline. Overrides: `user://`. How to use: [`docs/custom-editor-mvp.md`](docs/custom-editor-mvp.md). Design notes: [`docs/custom-editor-feasibility.md`](docs/custom-editor-feasibility.md). Schema: [`resources/scene_layout.example.json`](resources/scene_layout.example.json).
...
`--selftest` exercises custom ground, painted water + collision, and path playback, then restores the approved map (exit code `0` on success).
