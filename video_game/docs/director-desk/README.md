...
- 水域可为可缩放矩形或套索多边形，每区有独立流向（可画多条流向线，按距离加权合成）、速度、材质和碰撞。
- 普通图片元素支持 PS 式抓取移动、四角缩放和外圈旋转；中键平移不会打断当前操作，变换值可撤销并随场景保存。
- 场景支持多个角色；只有选中角色显示路线，各路线独立速度并在播放时并行。
- 环境支持早晨、中午、傍晚和夜晚；夜晚环境亮度可从全黑调到旧版默认亮度，冷色月光与偏白闪电作为独立光源，即使环境亮度为零也能照出场景。风具有方向、风力与 1×–20× 独立风速，每轮会重新随机风丝位置与轮廓；降雨强度与密度可分别调整，每轮雨滴会在降雨区域内部重新选择随机落点，单滴尺寸与对应水花大小联动。
